...
const DoctorCard = ({ doctor }) => {
  ...
  const formatSpecialties = () => {
    ...
    if (Array.isArray(doctor.specialities)) {
      // Handle the case where specialities is an array of objects with "name" property
      return doctor.specialities
        .map((specialty) =>
          typeof specialty === "object" && specialty.name
            ? specialty.name
            : specialty
        )
        .join(", ");
    ...
    if (typeof doctor.specialities === "string") {
      return doctor.specialities;
    }
  ...
  const handleBookAppointment = () => {
    alert(
      `Appointment request submitted for Dr. ${doctor.name}. You will receive a confirmation shortly.`
    );
  };
  ...
  return (
    <div
      ...
      className="bg-white rounded-lg shadow p-4 flex flex-col md:flex-row"
    >
      <div className="md:w-1/4 mb-4 md:mb-0 flex items-center justify-center">
        {imageError ? (
          ...
          <div
            className={`w-24 h-24 rounded-full ${avatarBgColor} flex items-center justify-center text-white font-bold text-xl`}
          >
          ...
          <img
            src={`/api/placeholder/${200 + (doctor.id % 10) * 20}/${
              200 + (doctor.id % 8) * 20
            }`}
            alt={`Dr. ${doctor.name}`}
      ...
      <div className="md:w-3/4">
        <h3 data-testid="doctor-name" className="text-lg font-semibold">
          {doctor.name}
        </h3>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          >
            {doctor.fees || `₹${doctor.fee} Consultation Fee`}
          </div>

...
function App() {
  ...
  useEffect(() => {
    if (doctors.length > 0) {
      ...
      if (params.specialties)
        setSelectedSpecialties(params.specialties.split(","));
  ...
  const applyFilters = (search, consultation, specialties, sort) => {
    ...
    if (specialties.length > 0) {
      result = result.filter((doctor) => {
        // Check if doctor has specialities property and it's an array
        if (!doctor.specialities || !Array.isArray(doctor.specialities)) {
          return false;
        }

        // Extract specialty names from the doctor's specialities array
        const doctorSpecialtyNames = doctor.specialities.map((spec) =>
          typeof spec === "object" && spec.name ? spec.name : spec
        );

        // Check if any selected specialty matches the doctor's specialties
    ...
    if (sort) {
      if (sort === "fees") {
        result.sort((a, b) => {
          ...
          return feeA - feeB; // Sort low to high
        });
        ...
        result.sort((a, b) => {
          ...
          return expB - expA; // Sort high to low
        });
      }
    }
          ...
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="md:w-1/2 mb-6 md:mb-0">
              ...
              <p className="text-blue-100">
                Connect with top specialists for your healthcare needs
              </p>
            </div>
            <div className="md:w-1/2 flex justify-center md:justify-end">
              <div className="relative w-64 h-48">
                ...
                >
                  ...
                  />
                  <path
                    d="M95,40 Q100,35 105,40 T115,40"
                    stroke="#ffffff"
                    strokeWidth="2"
                    fill="none"
                  />
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-4 md:mb-0">
              <p className="text-sm text-gray-300">
                © 2025 Doctor Finder. All rights reserved.
              </p>
            </div>
